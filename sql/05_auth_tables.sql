...
-- 2. SESSOES ──────────────────────────────────────────────────
--  expira_em é TEXT no formato 'YYYY-MM-DD HH:MM:SS' por compatibilidade
--  com o código atual (auth.py:35 faz datetime.strptime). Migrar para
--  TIMESTAMPTZ exigiria mudança no Python — registrado em /BUGS.md.
CREATE TABLE IF NOT EXISTS sessoes (
    id          BIGSERIAL    PRIMARY KEY,
    token       TEXT         NOT NULL UNIQUE,    -- uuid4().hex (32 chars)
    usuario_id  BIGINT       NOT NULL REFERENCES usuarios(id) ON DELETE CASCADE,
    criado_em   TIMESTAMPTZ  NOT NULL DEFAULT NOW(),
    expira_em   TEXT         NOT NULL            -- formato 'YYYY-MM-DD HH:MM:SS'
);
